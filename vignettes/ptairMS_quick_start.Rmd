...
```{r Align patient}
eSet <- alignSamples(exhaledSet, group="subfolder", fracGroup = 1, bgThreshold = 0 )
```
...
knitr::kable(head(features[names(sort(abs(dim1),decreasing = TRUE)),c("vocDB_ion_formula","vocDB_name_iupac")]))
```
...
```{r }
eSet <- alignSamples(bacteriaSet, group="subfolder", fracGroup = 1, bgThreshold = 2 )
```
