...
## Imputation 
To impute missing values, `ptairMS` returns back to the raw data, and compute with the same method as `detectPeak` the missing values without limit of detection. 
...
## Annotation
`ptairMS` provides putative annotations based on the mass ions matching to [Human Breathomics Database](https://hbdb.cmdm.tw)  [[@kuo_human_2020]](https://doi.org/10.1093/database/baz139), with the `annotateVOC` function. Applied to an expression set, it writes in the feature metadata all possible formulas, names, and the isotopes for nuclides C13, O17 and O18. 
...
You can now use statistical R packages such as `ropls` to perform PCA or further statistical analyses.
```{r imageF}
...
This research was conducted by Camille Roquencourt, with the contributions from Paul Zheng, Pierrick Roger-Mele and Etienne A. Thévenot ([**Data Sciences for Deep Phenotyping and Precision Medicine team**](http://etiennethevenot.pagesperso-orange.fr/) at CEA), in collaboration with Stanislas Grassin-Delyle, Hélène Salvator, Emmanuel Naline, Philippe Devillier and Louis-Jean Couderc ([**Exhalomics platform**](https://www.pluginlabs-universiteparissaclay.fr/fr/entity/915199-exhalomics-plateforme-danalyse-de-lair-expire) at the Foch Hospital) within the [**SoftwAiR project**](https://anr.fr/Projet-ANR-18-CE45-0017) funded by the Agence Nationale de la Recherche (ANR-18-CE45-0017 grant).
